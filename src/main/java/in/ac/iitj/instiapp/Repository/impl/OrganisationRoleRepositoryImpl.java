...
public class OrganisationRoleRepositoryImpl implements OrganisationRoleRepository {
    ...
    public Long existOrganisationRole(String organisationName, String roleName) {
        return jdbcTemplate.queryForObject("select COALESCE(MAX(r.organisation_id), -1) from organisation_role r " + "join organisation o on o.id = r.organisation_id join users u on o.user_id = u.id " + "where r.role_name = ? and u.user_name = ?", Long.class, roleName, organisationName);
    }
